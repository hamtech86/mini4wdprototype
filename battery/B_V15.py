...
APP_NAME = "Mini4WD Battery Analyzer V4-Core Rev3"
...
class Mode1(QWidget):
    ...
    def init_ui(self):
        ...
        self.btn_start.clicked.connect(self.start_pause_toggle)
        self.btn_reset = QPushButton("Reset ◀◀")
        self.btn_reset.clicked.connect(self.reset_discharge)
        ...
        self.label_remaining=QLabel("Estimated Remaining: 0 s")
    ...
    # --------------------------
    # 放電開始 / Pause / Resume 遷移
    # --------------------------
    def start_pause_toggle(self):

        if self.main.discharge_state in ["IDLE","STOPPED","DONE"]:
            self.main.send_cmd("START")
            self.main.discharge_time = 0
            self.main.discharge_state = "RUN"
            self.btn_start.setText("Pause ■")
        elif self.main.discharge_state=="RUN":
            self.main.send_cmd("PAUSE")
            self.main.discharge_state="PAUSE"
            self.btn_start.setText("Resume ▶")
        elif self.main.discharge_state=="PAUSE":
    ...
    def reset_discharge(self):
        ...
        self.main.discharge_time=0
        self.main.discharge_state="IDLE"
    ...
    def update_data(self,v,c,p,pwm):
        ...
        if self.main.discharge_state=="RUN":
            if self.main.last_update is not None:
                self.main.discharge_time += now - self.main.last_update
            self.main.last_update=now
        else:
        ...
        target_voltage=0.9
        remaining=0
        if self.main.discharge_state=="RUN" and v>target_voltage:
            remaining=int((v-target_voltage)/v * self.main.discharge_time)
        self.label_remaining.setText(f"Estimated Remaining: {remaining} s")
