...
class MotorBreakInUI:
    ...
    def build_ui(self):
        ...
        labels = [('Device', self.device_var), ('State', self.state_var),
                  ('ACS1', self.acs1_var), ('ACS2', self.acs2_var),
                  ('A3', self.a3_var), ('A4', self.a4_var), ('A5', self.a5_var),
                  ('A4 Voltage', self.a4v_var), ('A5 Voltage', self.a5v_var),
                  ('Motor Voltage', self.motorv_var), ('PWM', self.pwm_var),
                  ('TI', self.ti_var), ('SI', self.si_var), ('RPM', self.rpm_var)]
        ...
        ttk.Button(frm, text='STOP', command=lambda: self.send('STOP')).grid(row=r, column=1)
        ...
        ttk.Button(frm, text='SET PWM',
                   command=lambda: self.send(f'PWM={self.pwm_entry.get()}')).grid(row=r + 1, column=1)
        ...
        while self.running:
            try:
                ...
                if line.startswith('DATA,'):
                    ...
                    acs1 = int(p[2]); acs2 = int(p[3])
                    a3 = int(p[4]); a4 = int(p[5]); a5 = int(p[6])
                    ...
                    si = 100.0 if len(self.ti_history) < 5 else max(0.0, 100.0 - pstdev(self.ti_history))
